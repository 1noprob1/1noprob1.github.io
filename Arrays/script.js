...
const treOrdklasser = ordbok.filter(ordklasse => ordklasse.ordklasser.length >= 3);
...
const hoyPoppuler = ordbok.find(populariteten => populariteten.popularitet > 85);
console.table(hoyPoppuler)
...
const ordArray = ordbok.map(ordene => ordene.ord);
...
const antOrdklasser = ordbok.map(klassseord => 'ord: "' + klassseord.ord + '", antallOrdklasser: ' + klassseord.ordklasser.length);
console.log(antOrdklasser);
...
const totalOrdklasser = ordbok.reduce((sum, ordklasse) => sum + ordklasse.ordklasser.length, 0);
console.log(totalOrdklasser);
...
const popGjennomsnitt = ordbok.reduce((sum, poppuler) => sum + poppuler.popularitet, 0) / ordbok.length;
console.log(popGjennomsnitt)
...
const harVerb = ordbok.some(ord => ord.ordklasser.some(ordklasse.type === "verb"));
console.log(harVerb);
